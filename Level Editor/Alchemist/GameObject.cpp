...
GameObject::GameObject(IDirect3DTexture9 * t) : width(100), height(100), position(0,0,1), renderPosition(0,0,0), identifier(""),
	velocity(0,0,0), acceleration(0,0,0), texture(NULL),
	actorShader(0), assetIndex(-1), numSpriteCols(1), numSpriteRows(1), curSpriteCol(0), curSpriteRow(0), flipSprite(FALSE), rotation(0.0f),
	hMatrix(NULL), hTexture(NULL), hNumSpriteCols(NULL), hNumSpriteRows(NULL), hCurSpriteCol(NULL), hCurSpriteRow(NULL), hFlipSprite(NULL), hTechnique(NULL)
{
	texture = t;
	directory = "asdfasdfasdf";
	ID = currentID++;
}
...
int GameObject::renderFrame(long time, D3DXMATRIX * trans)
{
	...
	else
	{
		...
		D3DXMatrixTranslation(&camTrans, -camera->getPosition().x, -camera->getPosition().y, position.z);
...
IDirect3DTexture9 * GameObject::getTexture() { return texture; }

#pragma region Set Functions
void GameObject::setPosition(D3DXVECTOR3 pos) { position = D3DXVECTOR3(pos.x*0.01f, pos.y*0.01f, pos.z); }
...
void GameObject::setTexture(IDirect3DTexture9 * t) { if (t) texture = t; }
#pragma endregion
