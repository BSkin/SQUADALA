...
HRESULT MeshHierarchy::CreateFrame(LPCSTR Name, LPD3DXFRAME *retNewFrame)
{
    ...
    *retNewFrame = newFrame;
	...
	if (Name && strlen(Name))
	{
		newFrame->Name=Utility::DuplicateCharString(Name);	
		Utility::DebugString("Added frame: "+ToString(Name)+"\n");
	}
	else
	{
		Utility::DebugString("Added frame: no name given\n");
	}
    return S_OK;
}
...
{    
	...
	*retNewMeshContainer = 0;
	...
	if (Name && strlen(Name))
	{
		newMeshContainer->Name=Utility::DuplicateCharString(Name);
		Utility::DebugString("Added mesh: "+ToString(Name)+"\n");
	}
	else
	{
		Utility::DebugString("Added Mesh: no name given\n");
	}
	...
	if (numMaterials>0)
	{
		...
		for(DWORD i = 0; i < numMaterials; ++i)
		{
			...
			if(materials[i].pTextureFilename)
			{
				...
				if (Utility::FindFile(&texturePath))
				{
					...
					{
						Utility::DebugString("Could not load texture: "+texturePath+"\n");					
					}
				}
				else
				{
					Utility::DebugString("Could not find texture: "+ToString(materials[i].pTextureFilename)+"\n");					
				}
			}
		}
	}
	...
	if (pSkinInfo)
	{
		...
		Utility::DebugString("Mesh has skinning info.\n Number of bones is: "+ToString(numBones)+"\n");
	...
	if (effectInstances)
	{
		if (effectInstances->pEffectFilename)
			Utility::DebugString("This .x file references an effect file. Effect files are not handled by this demo\n");
	}
